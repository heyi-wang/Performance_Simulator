#!/usr/bin/env python3
"""Sweep dw_conv2d input size and thread count for the dwconv simulator.

Aligned with kernel/pooling/parametric_sweep.py so the resulting CSV is
consumable by the existing matmul/pooling plotting helpers.
"""
...
DW_DIM_RE = re.compile(
    r"^\s*static\s+const\s+int\s+(DW_[CHW]|DW_KH|DW_KW|DW_PAD|DW_STRIDE)\s*=\s*(\d+)\s*;"
)
...
def read_defaults() -> dict[str, int]:
    ...
    dw_config_path = REPO_ROOT / "kernel" / "dw_conv2d" / "dw_conv2d_config.h"
    name_to_key = {
        "DW_C": "channels",
        "DW_H": "height",
        "DW_W": "width",
    ...
    for line in dw_config_path.read_text().splitlines():
        match = DW_DIM_RE.match(line)
        if match:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Sweep dw_conv2d input size and threads, then plot elapsed cycles."
        )
    )
    parser.add_argument(
        "--max-workers",
    ...
    parser.add_argument(
        "--kernel-h",
        type=int,
        default=defaults["kernel_h"],
        help=f"Convolution kernel height. Default: {defaults['kernel_h']}.",
    )
    parser.add_argument(
        "--kernel-w",
        type=int,
        default=defaults["kernel_w"],
        help=f"Convolution kernel width. Default: {defaults['kernel_w']}.",
    )
    parser.add_argument(
        "--pad",
        type=int,
        default=defaults["pad"],
        help=f"Symmetric zero-padding. Default: {defaults['pad']}.",
    )
    parser.add_argument(
        "--stride",
        type=int,
        default=defaults["stride"],
        help=f"Convolution stride. Default: {defaults['stride']}.",
    )
    parser.add_argument(
        "--max-inflight-vec",
        type=int,
    ...
    parser.add_argument(
        "--binary",
        default=str(REPO_ROOT / "kernel" / "build" / "dw_conv2d_sim"),
        help="Path to the dwconv simulator binary.",
    )
    parser.add_argument(
        "--make-dir",
        default=str(REPO_ROOT / "kernel"),
        help="Directory containing the dwconv Makefile.",
    )
    parser.add_argument(
        "--csv",
        default=str(REPO_ROOT / "kernel" / "dw_conv2d" / "parametric_sweep.csv"),
        help="Output CSV path.",
    )
    parser.add_argument(
        "--png",
        default=str(REPO_ROOT / "kernel" / "dw_conv2d" / "parametric_sweep.png"),
...
def rebuild_binary(make_dir: Path, vec_accels: int) -> None:
    ...
    commands = (
        ["make", "clean"],
        ["make", "dw_conv2d", f"EXTRA_CXXFLAGS={extra}"],
    )
...
def run_point(
    ...
    command = [
        ...
        "--height", str(height),
        "--width", str(width),
        "--kernel-h", str(kernel_h),
        "--kernel-w", str(kernel_w),
        "--pad", str(pad),
        "--stride", str(stride),
    ]
    ...
    print(
        "[sweep] running "
        f"shape={channels}x{height}x{width} k={kernel_h}x{kernel_w} workers={workers}",
        flush=True,
    )
...
def write_csv(rows: list[dict[str, int | str]], csv_path: Path) -> None:
    ...
    fieldnames = [
        ...
        "build_ok",
        "run_ok",
        # Dwconv-specific aliases for humans and ad-hoc tooling.
        "size_label",
        "dw_channels",
        "dw_height",
        "dw_width",
...
def plot_rows(rows: list[dict[str, int | str]], png_path: Path) -> None:
    ...
    ax.set_title(
        f"DwConv2d Performance Simulation (input size vs threads), {hardware_label}",
        fontweight="bold",
    )
...
def main() -> int:
    ...
    if args.kernel_h < 1 or args.kernel_w < 1:
        parser.error("kernel dimensions must be >= 1")
    if args.stride < 1:
                    ...
                    rows.append(
                        {
                            ...
                            "dw_height": height,
                            "dw_width": width,
                            "dw_kernel_h": args.kernel_h,
                            "dw_kernel_w": args.kernel_w,
                            "dw_pad": args.pad,
                            "dw_stride": args.stride,
                            "workers": workers,
