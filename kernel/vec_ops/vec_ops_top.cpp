...
VecOpsTop::VecOpsTop(sc_module_name name)
    : sc_module(name),
      ...
      vec_acc("vec_acc",
              static_cast<size_t>(VOP_VEC_ACC_INSTANCES),
              VOP_ACC_QUEUE_DEPTH),
      noc("noc"),
      memory("memory",
             VOP_MEM_BASE_LAT,
             VOP_MEM_BW,
             static_cast<uint64_t>(VOP_VEC_ACC_INSTANCES))
{
...
VecOpsSimulationStats VecOpsTop::collect_stats() const
{
    ...
    const int n_tiles =
        static_cast<int>(ceil_div_u64(static_cast<uint64_t>(spatial), tile_cap));
    ...
    stats.expected_vec_calls = static_cast<uint64_t>(VOP_C) * static_cast<uint64_t>(n_tiles);
    ...
    for (int t = 0; t < n_tiles; ++t)
    {
        uint64_t vl = std::min<uint64_t>(
            tile_cap,
            static_cast<uint64_t>(spatial) - static_cast<uint64_t>(t) * tile_cap);
        per_chan_rd += vop_rd_bytes(VOP_SELECTED_OP, vl);
        per_chan_wr += vop_wr_bytes(VOP_SELECTED_OP, vl);
    }
    stats.expected_rd_bytes =
        static_cast<uint64_t>(VOP_C) * (per_chan_rd + extra_rd_per_channel);
    stats.expected_wr_bytes = static_cast<uint64_t>(VOP_C) * per_chan_wr;
