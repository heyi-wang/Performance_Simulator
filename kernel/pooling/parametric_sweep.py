...
POOL_DIM_RE = re.compile(
    r"^\s*static\s+const\s+int\s+(POOL_[CHW])\s*=\s*(\d+)\s*;"
)
...
def read_defaults() -> tuple[int, int, int, int, int]:
    vec_accels = 1
    vec_bytes = 64
    dims = {"POOL_C": 32, "POOL_H": 64, "POOL_W": 64}
    ...
    for line in hardware_path.read_text().splitlines():
        ...
        if match:
            vec_accels = int(match.group(1))
            continue
        match = CONFIG_VECTOR_CAP_RE.match(line)
        if match:
            vec_bytes = int(match.group(1))

    pooling_config_path = REPO_ROOT / "kernel" / "pooling" / "pooling_config.h"
    for line in pooling_config_path.read_text().splitlines():
        match = POOL_DIM_RE.match(line)
        if match:
            name, value = match.groups()
            dims[name] = int(value)

    return vec_accels, vec_bytes, dims["POOL_C"], dims["POOL_H"], dims["POOL_W"]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=[default_vec_accels],
        help=(
            "Comma-separated compile-time vector accelerator counts. "
            f"Default: {default_vec_accels} from config/hardware_config.h."
        ),
    )
    parser.add_argument(
        "--vec-bytes",
        ...
        help=(
            ...
            f"Default: {vec_bytes} from config/hardware_config.h."
        ),
    )
    parser.add_argument(
        "--base-channels",
        type=int,
        default=channels,
        help=f"Base channel count before --channel-multipliers. Default: {channels}.",
    )
    parser.add_argument(
        "--base-height",
        type=int,
        default=height,
        help=f"Base input height before --size-multipliers. Default: {height}.",
    )
    parser.add_argument(
        "--base-width",
        type=int,
        default=width,
        help=f"Base input width before --size-multipliers. Default: {width}.",
    )
    parser.add_argument(
        "--max-inflight-vec",
        type=int,
    ...
    parser.add_argument(
        "--binary",
        default=str(REPO_ROOT / "kernel" / "build" / "pooling_sim"),
        help="Path to the pooling simulator binary.",
    )
    parser.add_argument(
        "--make-dir",
        default=str(REPO_ROOT / "kernel"),
        help="Directory containing the pooling Makefile.",
    )
    parser.add_argument(
        "--csv",
        default=str(REPO_ROOT / "kernel" / "pooling" / "parametric_sweep.csv"),
        help="Output CSV path.",
    )
    parser.add_argument(
        "--png",
        default=str(REPO_ROOT / "kernel" / "pooling" / "parametric_sweep.png"),
...
def rebuild_binary(make_dir: Path, vec_accels: int) -> None:
    ...
    commands = (
        ["make", "clean"],
        ["make", "pooling", f"EXTRA_CXXFLAGS={extra}"],
    )
...
def write_csv(rows: list[dict[str, int | str]], csv_path: Path) -> None:
    ...
    fieldnames = [
        # Matmul full_sweep.csv-compatible columns consumed by
        # kernel/matmul/plot_sweep.py. For pooling, gemm_m/k/n map to
        # channels/height/width so those dimensions stay numeric to the
        # plotting tool without requiring plot_sweep.py changes.
        "tile_m",
        "tile_k",
        "tile_n",
        "mat_latency",
        ...
        "build_ok",
        "run_ok",
        # Pooling-specific aliases for humans and ad-hoc tooling.
        "size_label",
        "pool_channels",
        "pool_height",
        "pool_width",
